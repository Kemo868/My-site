...
<p>${star.biography}</p>
<h3>Achievements</h3>
<ul>${star.achievements.map(a => `<li>${a.title} (${a.year}):
${a.description}</li>`).join('')}</ul>
<h3>Contributions</h3>
<ul>${star.contributions.map(c => `<li>${c.title}:
${c.description}</li>`).join('')}</ul>
`;
};
...
const achievements =
document.getElementById('achievements').value.trim().split('\n').map(line
=> {
const [title, year, description] = line.split(',');
return { title, year: parseInt(year), description };
});
const contributions =
document.getElementById('contributions').value.trim().split('\n').map(line
=> {
